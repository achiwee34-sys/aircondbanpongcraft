...
function closeSheet(name){
  ...
  if (ov) setTimeout(()=>ov.classList.remove('open'),350);
}

// ============================================================
// GOOGLE SHEETS
// ============================================================
function saveGSUrl(){db.gsUrl=document.getElementById('gs-url')?.value.trim();saveDB();showToast('✅ บันทึก URL แล้ว');}
...
async function testGS(){
  ...
  const r=document.getElementById('gs-result');
  if(!url){r.textContent='❌ กรุณาใส่ URL';return;}
  r.innerHTML='⏳ กำลังทดสอบ...';
  try{
    await fetch(url,{method:'GET',mode:'no-cors'});
    db.gsUrl=url; saveDB();
    r.innerHTML='<span style="color:var(--ok)">✅ บันทึก URL แล้ว พร้อม Sync</span>';
  }catch(e){
    r.innerHTML='<span style="color:var(--warn)">❌ URL ไม่ถูกต้อง: '+e.message+'</span>';
  }
}
...
async function syncAll(){
  const url=db.gsUrl||document.getElementById('gs-url').value.trim();
  if(!url){alert('กรุณาตั้งค่า URL ก่อน');return;}
  const r=document.getElementById('gs-result');r.innerHTML='⏳ กำลัง Sync...';
  try{
    await fetch(url,{method:'POST',mode:'no-cors',body:JSON.stringify({action:'all',tickets:db.tickets,machines:db.machines,users:db.users.map(u=>({id:u.id,name:u.name,username:u.username,role:u.role,dept:u.dept||'',tel:u.tel||'',contact:u.contact||''}))}),headers:{'Content-Type':'application/json'}});
    r.innerHTML='<span style="color:var(--ok)">✅ Sync เสร็จแล้ว '+nowStr()+'<br><span style="font-size:0.75rem;color:var(--muted)">เปิด Google Sheets เพื่อตรวจสอบข้อมูล</span></span>';
  }catch(e){
    r.innerHTML='<span style="color:var(--warn)">❌ '+e.message+'</span>';
  }
}
let _syncCount = 0;
...
function copyGSCode(el){navigator.clipboard?.writeText(el.textContent.trim()).then(()=>{el.style.borderColor='var(--ok)';setTimeout(()=>el.style.borderColor='',2000);});}
...
function getMeasurements() {
  return {
    tempBefore: document.getElementById('m-temp-before')?.value||'',
    tempAfter:  document.getElementById('m-temp-after')?.value||'',
    ampBefore:  document.getElementById('m-amp-before')?.value||'',
    ampAfter:   document.getElementById('m-amp-after')?.value||'',
    psiLoBefore:document.getElementById('m-psi-lo-before')?.value||'',
    psiLoAfter: document.getElementById('m-psi-lo-after')?.value||'',
    psiHiBefore:document.getElementById('m-psi-hi-before')?.value||'',
    psiHiAfter: document.getElementById('m-psi-hi-after')?.value||'',
  };
}
...
// ============================================================
function populatePMDeptSelect() {
  const sel = document.getElementById('pm-dept-select');
...
function generatePMTicketsByDept() {
  const dept = document.getElementById('pm-dept-select')?.value || '';
  if (!dept) { showToast('⚠️ กรุณาเลือกแผนกก่อน'); return; }
  generatePMTickets(dept);
}
...
function _pmPlanDates(startDate, freq) {
  const dates = [startDate];
  const d = new Date(startDate);
  if (freq === 'monthly') {
    for (let i = 1; i < 3; i++) {
      const nd = new Date(d); nd.setMonth(nd.getMonth() + i);
      dates.push(nd.toISOString().split('T')[0]);
    }
  } else if (freq === 'quarterly') {
    for (let i = 1; i < 4; i++) {
      const nd = new Date(d); nd.setMonth(nd.getMonth() + i * 3);
      dates.push(nd.toISOString().split('T')[0]);
    }
...
function openPMPlanSheet() {
  // เปลี่ยนเป็นหน้าเต็มแทน bottom sheet
  openPMPlanPage();
}
...
// LINE NOTIFY
// ============================================================
function saveLineNotifySettings() {
  const tokenAdmin = document.getElementById('line-token-admin')?.value.trim();
  const tokenTech  = document.getElementById('line-token-tech')?.value.trim();
  const evNew    = document.getElementById('ln-ev-new')?.checked ?? true;
  const evAccept = document.getElementById('ln-ev-accept')?.checked ?? true;
  const evDone   = document.getElementById('ln-ev-done')?.checked ?? true;
  db.lineNotify = { tokenAdmin, tokenTech, evNew, evAccept, evDone };
  saveDB();
  const statusEl = document.getElementById('line-notify-status');
  if (statusEl) {
    const hasToken = tokenAdmin || tokenTech;
    statusEl.textContent = hasToken ? '● เปิดใช้งาน' : 'ยังไม่ตั้งค่า';
    statusEl.style.color  = hasToken ? '#06C755' : '#9ca3af';
  }
  showToast('✅ บันทึกตั้งค่า Line Notify แล้ว');
}

function toggleLineTokenVisibility(type) {
  const inp = document.getElementById('line-token-' + type);
  if (inp) inp.type = inp.type === 'password' ? 'text' : 'password';
}

async function testLineNotify() {
  const r = document.getElementById('line-test-result');
  const token = document.getElementById('line-token-admin')?.value.trim()
              || document.getElementById('line-token-tech')?.value.trim();
  if (!token) { if(r) r.innerHTML='<span style="color:#ef4444">❌ กรุณากรอก Token ก่อน</span>'; return; }
  if (r) r.innerHTML = '⏳ กำลังทดสอบ...';
  try {
    ...
    if (res.ok) {
      if(r) r.innerHTML='<span style="color:#06C755;font-weight:700">✅ ส่งสำเร็จ! ตรวจสอบใน LINE ได้เลย</span>';
    } else {
      if(r) r.innerHTML='<span style="color:#ef4444">❌ Token ไม่ถูกต้อง ('+res.status+')</span>';
    }
  } catch(e) {
    if(r) r.innerHTML='<span style="color:#f59e0b">⚠️ ทดสอบไม่ได้ผ่าน Browser (CORS) — ต้องใช้ผ่าน Backend proxy</span>';
  }
}
...
function initLineNotifyUI() {
  const ln = db.lineNotify;
  if (!ln) return;
  const adminInp = document.getElementById('line-token-admin');
  const techInp  = document.getElementById('line-token-tech');
  if (adminInp) adminInp.value = ln.tokenAdmin || '';
  if (techInp)  techInp.value  = ln.tokenTech  || '';
  const evNew    = document.getElementById('ln-ev-new');
  const evAccept = document.getElementById('ln-ev-accept');
  const evDone   = document.getElementById('ln-ev-done');
  if (evNew)    evNew.checked    = ln.evNew    ?? true;
  if (evAccept) evAccept.checked = ln.evAccept ?? true;
  if (evDone)   evDone.checked   = ln.evDone   ?? true;
  const statusEl = document.getElementById('line-notify-status');
  if (statusEl) {
    const hasToken = ln.tokenAdmin || ln.tokenTech;
    statusEl.textContent = hasToken ? '● เปิดใช้งาน' : 'ยังไม่ตั้งค่า';
    statusEl.style.color  = hasToken ? '#06C755' : '#9ca3af';
  }
}
...
function openSignaturePad(tid, type) {
  ...
  const labels = {
    tech_done:       { title:'✍️ ช่างเซ็นชื่อยืนยันงานซ่อม',  sub:'เซ็นชื่อเพื่อยืนยันว่าซ่อมเสร็จสมบูรณ์', color:'#0369a1' },
    reporter_verify: { title:'✍️ ผู้แจ้งเซ็นชื่อตรวจรับงาน',  sub:'เซ็นชื่อเพื่อยืนยันว่าแอร์ทำงานได้ปกติ', color:'#059669' },
    admin_close:     { title:'✍️ Admin เซ็นชื่อปิดงาน',       sub:'เซ็นชื่อเพื่อปิดงานอย่างเป็นทางการ',    color:'#7c3aed' },
  };
  const lbl = labels[type] || { title:'✍️ เซ็นชื่อ', sub:'', color:'#c8102e' };

  // สร้าง overlay + pad
  const ov = document.createElement('div');
  ov.id = 'sig-overlay';
  ov.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,0.6);z-index:9500;backdrop-filter:blur(4px);display:flex;align-items:flex-end;justify-content:center';

  const pad = document.createElement('div');
  pad.id = 'sig-pad';
  pad.style.cssText = 'width:100%;max-width:520px;background:white;border-radius:24px 24px 0 0;padding:20px 20px 32px;box-shadow:0 -8px 40px rgba(0,0,0,0.2)';
  pad.innerHTML = `
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
      <div>
        <div style="font-size:1rem;font-weight:900;color:#0f172a">${lbl.title}</div>
        <div style="font-size:0.72rem;color:#64748b;margin-top:2px">${lbl.sub}</div>
      </div>
      <button onclick="closeSignaturePad()" style="width:32px;height:32px;border-radius:50%;border:none;background:#f1f5f9;color:#64748b;font-size:1rem;cursor:pointer;display:flex;align-items:center;justify-content:center">✕</button>
    </div>
    <div style="margin:12px 0;background:#f8fafc;border:2px dashed #cbd5e1;border-radius:16px;overflow:hidden;position:relative">
      <canvas id="sig-canvas" style="width:100%;display:block;touch-action:none;cursor:crosshair"></canvas>
      <div id="sig-placeholder" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;pointer-events:none">
        <div style="text-align:center;color:#cbd5e1">
          <div style="font-size:2rem;margin-bottom:4px">✍️</div>
          <div style="font-size:0.78rem;font-weight:600">เซ็นชื่อในกรอบนี้</div>
        </div>
      </div>
    </div>
    <div style="display:flex;gap:8px;margin-top:12px">
      <button onclick="clearSignaturePad()" style="flex:1;padding:12px;background:#f1f5f9;color:#64748b;border:none;border-radius:12px;font-size:0.88rem;font-weight:700;cursor:pointer;font-family:inherit">
        🗑️ ล้าง
      </button>
      <button onclick="confirmSignature()" style="flex:2;padding:12px;background:linear-gradient(135deg,${lbl.color},${lbl.color}cc);color:white;border:none;border-radius:12px;font-size:0.9rem;font-weight:800;cursor:pointer;font-family:inherit;box-shadow:0 4px 14px ${lbl.color}44">
        ✅ ยืนยันลายเซ็น
      </button>
    </div>`;
  ...
  // setup canvas — ต้อง append ก่อน ถึงจะได้ clientWidth จริง (fix iPhone)
  const canvas = document.getElementById('sig-canvas');
  const dpr = Math.min(window.devicePixelRatio || 1, 2); // cap ที่ 2 เพื่อ performance
  const w = Math.max(280, pad.clientWidth - 40); // fallback ถ้า clientWidth = 0
  canvas.width  = w * dpr;
  ...
  const getPos = e => {
    ...
    return { x: (pt.clientX - rect.left) * (canvas.width / rect.width / dpr), y: (pt.clientY - rect.top) * (canvas.height / rect.height / dpr) };
  };
  canvas.addEventListener('mousedown',  e => { _sigDrawing=true; const p=getPos(e); _sigLastX=p.x;_sigLastY=p.y; document.getElementById('sig-placeholder').style.opacity='0'; });
  canvas.addEventListener('mousemove',  e => { if(!_sigDrawing)return; const p=getPos(e); _sigCtx.beginPath();_sigCtx.moveTo(_sigLastX,_sigLastY);_sigCtx.lineTo(p.x,p.y);_sigCtx.stroke(); _sigLastX=p.x;_sigLastY=p.y; });
  canvas.addEventListener('mouseup',    () => _sigDrawing=false);
  canvas.addEventListener('touchstart', e => { e.preventDefault(); _sigDrawing=true; const p=getPos(e); _sigLastX=p.x;_sigLastY=p.y; document.getElementById('sig-placeholder').style.opacity='0'; }, {passive:false});
  canvas.addEventListener('touchmove',  e => { e.preventDefault(); if(!_sigDrawing)return; const p=getPos(e); _sigCtx.beginPath();_sigCtx.moveTo(_sigLastX,_sigLastY);_sigCtx.lineTo(p.x,p.y);_sigCtx.stroke(); _sigLastX=p.x;_sigLastY=p.y; }, {passive:false});
...
function _drawQRSmall(text, canvas, m) {
  const size = canvas.width;
  const ctx = canvas.getContext('2d');
  ctx.fillStyle = '#f8fafc';
  ctx.fillRect(0, 0, size, size);
  const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size='+size+'x'+size+'&data=' + encodeURIComponent(text) + '&margin=2';
  const img = new Image();
...
function printDeptQR() {
  ...
  Object.entries(deptMap).sort(([a],[b])=>a.localeCompare(b)).forEach(([dept, list]) => {
    bodyHtml += `
      <div style="break-inside:avoid;margin-bottom:24px">
        <div style="background:#7c3aed;color:white;padding:8px 14px;border-radius:8px 8px 0 0;font-size:12px;font-weight:800;display:flex;justify-content:space-between">
          <span>${dept}</span>
          <span>${list.length} เครื่อง</span>
        </div>
        <div style="display:grid;grid-template-columns:repeat(${Math.min(COLS,list.length)},1fr);gap:8px;padding:10px;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;background:white">
          ${list.map(m => `
            <div style="text-align:center;border:1px solid #f0f0f0;border-radius:8px;padding:6px 4px;break-inside:avoid">
              <img loading="lazy" decoding="async" src="https://api.qrserver.com/v1/create-qr-code/?size=90x90&data=${encodeURIComponent(m.id)}&margin=2"
                   width="80" height="80" style="display:block;margin:0 auto;border-radius:4px"/>
              <div style="font-family:monospace;font-size:7px;font-weight:800;color:#7c3aed;margin-top:4px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${m.serial||m.id.replace('csv_','')}</div>
              <div style="font-size:6px;color:#94a3b8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${(m.name||'').slice(0,18)}</div>
            </div>`).join('')}
        </div>
      </div>`;
  });
  ...
  win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8">
    <title>QR Code รายแผนก — SCG AIRCON BP</title>
    <style>
      * { box-sizing:border-box; margin:0; padding:0; }
      body { font-family:'Noto Sans Thai',Arial,sans-serif; background:white; padding:16px; }
      @media print {
        body { padding:8px; }
    ...
    <div class="no-print" style="background:#7c3aed;color:white;padding:10px 16px;border-radius:10px;margin-bottom:16px;display:flex;align-items:center;justify-content:space-between">
      <div style="font-size:14px;font-weight:800">⬛ QR Code รายแผนก — SCG AIRCON BP &nbsp;·&nbsp; ${machines.length} เครื่อง</div>
      <button onclick="window.print()" style="background:white;color:#7c3aed;border:none;border-radius:8px;padding:8px 18px;font-weight:800;cursor:pointer;font-size:13px">🖨️ พิมพ์</button>
    </div>
    ${bodyHtml}
  </body></html>`);
  win.document.close();
