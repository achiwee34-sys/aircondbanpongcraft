 ...
 * @param {object} ticket
 */
async function deleteTicketPhotos(ticket) {
  if (!FSstorage || !ticket?.id) return;
  const allPhotos = [...(ticket.photosBefore||[]), ...(ticket.photosAfter||[])];
  for (const url of allPhotos) {
    if (url && url.startsWith('https://')) {
      try {
        await FSstorage.refFromURL(url).delete();
      } catch(e) {
        // ignore — อาจถูกลบไปแล้ว
      }
    }
  }
}
...
async function runPhotoMigrationUI() {
  ...
  await migrateAllPhotosToStorage(
    (current, total, tid) => {
      console.log(`[Migration] ${current}/${total}: ${tid}`);
      if (typeof showToast === 'function') {
