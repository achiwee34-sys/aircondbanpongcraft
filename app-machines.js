...
function renderMachineRows(list) {
  const html = _buildMachineRowsHtml(list);
  requestAnimationFrame(() => {
    const tbody = document.getElementById('mac-tbody');
    if (tbody) tbody.innerHTML = html;
    const countEl = document.getElementById('mac-count');
    if (countEl) countEl.textContent = list.length < db.machines.length
      ? `แสดง ${list.length} จาก ${db.machines.length} เครื่อง`
      : `ทั้งหมด ${db.machines.length} เครื่อง`;
  });
}
...
function renderDeptTabs() {
  // Dept tabs are now rendered inside applyMachineFilter (batched with rows in one rAF)
}
// ── Export Menu ──
...
function setMacView(v) {
  window._macView = v;
  // ใหม่: dashboard อยู่หน้าหลักเสมอ, table อยู่ใน sheet
  // function นี้เก็บไว้เพื่อ backward compat แต่ไม่ toggle อะไรแล้ว
  if (v === 'table') {
    openMacTableSheet();
  } else {
...
function toggleNoFuncLocSection() {
  // legacy — ใช้ popup แทน
  showNoEquipPopup();
}

function toggleDupEqSection() {
  // legacy — ใช้ popup แทน
  showDupEqPopup();
}
...
function openQRScannerOverlay() {
  // (fullscreen overlay scanner — kept for reference)
  // ปัจจุบันใช้ openQRScanner() ซึ่งใช้ qr-modal HTML แทน
  // Check camera support
  if (!navigator.mediaDevices?.getUserMedia) {
    showToast('⚠️ Browser นี้ไม่รองรับ Camera');
    return;
  }
  ...
  // Header
  ov.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px;background:rgba(0,0,0,0.8);flex-shrink:0">
      <div style="color:white;font-size:0.95rem;font-weight:800">📷 สแกน QR Code เครื่องแอร์</div>
      <button id="qr-close" style="width:34px;height:34px;border-radius:50%;background:rgba(255,255,255,0.15);border:none;color:white;font-size:1.1rem;cursor:pointer">✕</button>
    </div>
    <div style="flex:1;position:relative;overflow:hidden;display:flex;align-items:center;justify-content:center">
      <video id="qr-video-2" playsinline autoplay muted style="width:100%;height:100%;object-fit:cover"></video>
      <!-- Viewfinder -->
      <div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;pointer-events:none">
        <div style="position:relative;width:240px;height:240px">
          <!-- corners -->
          <div style="position:absolute;top:0;left:0;width:40px;height:40px;border-top:4px solid #fff;border-left:4px solid #fff;border-radius:4px 0 0 0"></div>
          <div style="position:absolute;top:0;right:0;width:40px;height:40px;border-top:4px solid #fff;border-right:4px solid #fff;border-radius:0 4px 0 0"></div>
          <div style="position:absolute;bottom:0;left:0;width:40px;height:40px;border-bottom:4px solid #fff;border-left:4px solid #fff;border-radius:0 0 0 4px"></div>
          <div style="position:absolute;bottom:0;right:0;width:40px;height:40px;border-bottom:4px solid #fff;border-right:4px solid #fff;border-radius:0 0 4px 0"></div>
          <!-- scan line -->
          <div id="qr-scanline" style="position:absolute;left:4px;right:4px;height:2px;background:linear-gradient(90deg,transparent,#c8102e,transparent);top:0;animation:qrScan 2s linear infinite"></div>
        </div>
      </div>
      <!-- overlay darkening outside viewfinder -->
      <div style="position:absolute;inset:0;background:rgba(0,0,0,0.45);clip-path:polygon(0 0,100% 0,100% 100%,0 100%,0 calc(50% - 120px),calc(50% - 120px) calc(50% - 120px),calc(50% - 120px) calc(50% + 120px),calc(50% + 120px) calc(50% + 120px),calc(50% + 120px) calc(50% - 120px),0 calc(50% - 120px));pointer-events:none"></div>
    </div>
    <div style="padding:12px 16px 28px;background:rgba(0,0,0,0.8);flex-shrink:0">
      <div id="qr-status-2" style="color:rgba(255,255,255,0.7);font-size:0.78rem;text-align:center;margin-bottom:12px">วางกล้องตรง QR Code บนป้ายเครื่องแอร์</div>
      <!-- Manual input fallback -->
      <div style="display:flex;gap:8px">
        <input id="qr-manual" type="text" placeholder="พิมพ์ Air ID / Serial เพื่อค้นหาแทน..." 
          style="flex:1;padding:10px 14px;border-radius:10px;border:1.5px solid rgba(255,255,255,0.2);background:rgba(255,255,255,0.1);color:white;font-size:0.85rem;font-family:inherit;outline:none"
          oninput="this.style.borderColor=this.value?'#c8102e':'rgba(255,255,255,0.2)'"
          onkeydown="if(event.key==='Enter')qrManualSearch(this.value)"/>
        <button onclick="qrManualSearch(document.getElementById('qr-manual').value)"
          style="padding:10px 16px;background:#c8102e;color:white;border:none;border-radius:10px;font-size:0.82rem;font-weight:700;cursor:pointer;font-family:inherit">ค้นหา</button>
      </div>
    </div>`;
  ...
  if (!document.getElementById('qr-style')) {
    ...
    s.id = 'qr-style';
    s.textContent = '@keyframes qrScan { 0%{top:4px} 50%{top:calc(100% - 6px)} 100%{top:4px} }';
  ...
  navigator.mediaDevices.getUserMedia({ video: { facingMode: 'environment' }, audio: false })
    .then(s => {
      stream = s;
      const video = ov.querySelector('#qr-video');
      video.srcObject = s;
      video.play();
        ...
        const scan = async () => {
          ...
          try {
            ...
            if (barcodes.length > 0) {
              ...
              ov.querySelector('#qr-status').innerHTML = `<span style="color:#4ade80">✅ สแกนได้: <b>${raw}</b></span>`;
              navigator.vibrate?.([100, 50, 100]);
              setTimeout(() => qrFoundMachine(raw, ov, stream), 600);
              return;
            }
          } catch(e) {}
          animFrame = requestAnimationFrame(scan);
        };
        video.addEventListener('loadedmetadata', () => { animFrame = requestAnimationFrame(scan); });
      } else {
        // BarcodeDetector not supported — show manual only
        ov.querySelector('#qr-status').innerHTML = '<span style="color:#fde68a">⚠️ Browser ไม่รองรับ auto-scan — พิมพ์ Air ID ด้านล่างแทนได้</span>';
      }
    })
    .catch(err => {
      ov.querySelector('#qr-status').innerHTML = '<span style="color:#fca5a5">⚠️ ไม่สามารถเปิดกล้องได้ — ' + (err.name === 'NotAllowedError' ? 'กรุณาอนุญาต Camera' : err.message) + '</span>';
    });
  ...
  ov.querySelector('#qr-close').onclick = closeScanner;
  ov.querySelector('#qr-manual').focus();

  // Manual search helper (closure)
  window._qrOv = ov;
  window._qrStream = stream;
  window._qrClose = closeScanner;
}
...
function onLookupSearch(q) {
  // ไม่ใช้แล้ว — legacy stub
}

function onLookupMachineSelect(mid) {
  onLookupMachineChange_byId(mid);
}

function onLookupMachineChange() {
  // legacy stub
}
...
function openMachineRequestsSheet() { openMachineRequestsPage(); }
...
function exportMachinesExcel() {
  if (typeof XLSX === 'undefined') { showToast('⚠️ XLSX library ไม่พร้อม'); return; }
  const machines = db.machines || [];
  const rows = machines.map((m,i) => ({
    '#': i+1,
    'Air ID': m.serial || m.id.replace('csv_',''),
    'ชื่อเครื่อง / ห้อง': m.name || '',
    'แผนก': m.dept || m.location || '',
    'BTU': m.btu || '',
    ...
    'รุ่น FCU': m.modelFCU || '',
    'Vendor': m.vendor || '',
    'Function Location': m.funcLoc || '',
    'Equipment No.': m.equipment || '',
    'วันติดตั้ง': m.install || '',
    'ระยะ PM (เดือน)': m.interval || '',
    'เพิ่มเมื่อ': (m.addedAt||'').substring(0,10),
  }));
  const ws = XLSX.utils.json_to_sheet(rows);
  // Column widths
  ws['!cols'] = [5,8,35,25,10,12,14,16,14,16,8,22,22,12,12,12].map(w=>({wch:w}));
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, 'เครื่องแอร์');
  const date = new Date().toISOString().substring(0,10);
  XLSX.writeFile(wb, `SCG_AIRCON_${date}.xlsx`);
  showToast(`✅ Export ${rows.length} เครื่อง สำเร็จ`);
}
...
function openMachineRequestsSheet_OLD() {
  const reqs = (db.machineRequests||[]).filter(r => r.status === 'pending');
  const ov = document.createElement('div');
  ov.style.cssText = 'position:fixed;inset:0;z-index:9200;background:rgba(0,0,0,0.5);backdrop-filter:blur(4px);display:flex;align-items:flex-end;justify-content:center';

  const sh = document.createElement('div');
  sh.style.cssText = 'width:100%;max-width:600px;background:#f8fafc;border-radius:22px 22px 0 0;max-height:90vh;display:flex;flex-direction:column;animation:slideUp 0.28s cubic-bezier(0.32,0.72,0,1)';

  const renderList = () => {
    const pending = (db.machineRequests||[]).filter(r => r.status === 'pending');
    sh.innerHTML = `
      <div style="display:flex;justify-content:center;padding:10px 0 0"><div style="width:40px;height:4px;background:#d1d5db;border-radius:99px"></div></div>
      <div style="padding:12px 16px 10px;border-bottom:1px solid #e5e7eb;background:white;border-radius:22px 22px 0 0;flex-shrink:0">
        <div style="display:flex;align-items:center;gap:10px">
          <div style="width:32px;height:32px;background:#0f172a;border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:0.9rem">📋</div>
          <div>
            <div style="font-size:0.95rem;font-weight:800;color:#0f172a">คำขอเพิ่มเครื่องแอร์</div>
            <div style="font-size:0.68rem;color:#64748b">รอการอนุมัติ ${pending.length} รายการ</div>
          </div>
          <button onclick="this.closest('[style*=inset]').remove()" style="margin-left:auto;background:#f1f5f9;border:none;border-radius:8px;width:30px;height:30px;cursor:pointer;font-size:1rem;display:flex;align-items:center;justify-content:center">✕</button>
        </div>
      </div>
      <div style="flex:1;overflow-y:auto;padding:12px 16px">
        ${pending.length === 0
          ? '<div style="text-align:center;padding:40px;color:#94a3b8;font-size:0.88rem">✅ ไม่มีคำขอที่รอดำเนินการ</div>'
          : pending.map(r => `
          <div style="background:white;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;margin-bottom:10px">
            <!-- Header -->
            <div style="background:#0f172a;padding:10px 14px;display:flex;align-items:center;justify-content:space-between">
              <div style="display:flex;align-items:center;gap:8px">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.7)" stroke-width="2" stroke-linecap="round"><rect x="2" y="3" width="20" height="8" rx="2"/><line x1="2" y1="7" x2="22" y2="7"/></svg>
                <span style="font-size:0.78rem;font-weight:700;color:white">${r.data.name}</span>
              </div>
              <span style="font-size:0.62rem;color:rgba(255,255,255,0.6);font-family:monospace">${r.serial}</span>
            </div>
            <!-- Info -->
            <div style="padding:10px 14px">
              <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px">
                <div style="font-size:0.72rem;color:#64748b"><span style="font-weight:600;color:#374151">แผนก:</span> ${r.data.dept||'—'}</div>
                <div style="font-size:0.72rem;color:#64748b"><span style="font-weight:600;color:#374151">BTU:</span> ${r.data.btu ? Number(r.data.btu).toLocaleString() : '—'}</div>
                <div style="font-size:0.72rem;color:#64748b"><span style="font-weight:600;color:#374151">Vendor:</span> ${r.data.vendor||'—'}</div>
                <div style="font-size:0.72rem;color:#64748b"><span style="font-weight:600;color:#374151">น้ำยา:</span> ${r.data.refrigerant||'—'}</div>
              </div>
              <div style="font-size:0.68rem;color:#94a3b8;margin-bottom:10px">
                📤 ขอโดย <strong style="color:#475569">${r.requestedByName}</strong> · ${(r.requestedAt||'').substring(0,16).replace('T',' ')}
              </div>
              <div style="display:flex;gap:8px">
                <button onclick="approveMachineReq('${r.id}')" style="flex:2;padding:10px;background:#0f172a;color:white;border:none;border-radius:10px;font-size:0.82rem;font-weight:700;cursor:pointer;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:6px">
                ...
                <button onclick="rejectMachineReq('${r.id}')" style="flex:1;padding:10px;background:#fff0f2;color:#c8102e;border:1.5px solid #fecaca;border-radius:10px;font-size:0.82rem;font-weight:700;cursor:pointer;font-family:inherit">
                  ปฏิเสธ
                </button>
              </div>
            </div>
  ...
  window.approveMachineReq = (reqId) => {
    ...
    db.machines.push(nm);
    req.status = 'approved';
    req.approvedBy = CU.id;
    req.approvedAt = nowStr();
    syncMachine(nm);
    saveDB();
    notifyUser(req.requestedBy, '✅ คำขอได้รับการอนุมัติ', `เครื่อง [${req.serial}] ${req.data.name} เพิ่มในระบบแล้ว`, null);
    showToast('✅ อนุมัติแล้ว — เพิ่มเครื่องแอร์ในระบบ');
    invalidateMacCache(); invalidateTkCache();
    renderMachines();
    renderList();
  };
  ...
  window.rejectMachineReq = (reqId) => {
    ...
    showAlert({
      icon: '❌', color: '#c8102e',
      title: 'ปฏิเสธคำขอ?',
      msg: `ปฏิเสธการเพิ่ม <strong>${req.data.name}</strong><br>
        <span style="font-size:0.78rem;color:#64748b">ช่างจะได้รับแจ้งว่าคำขอถูกปฏิเสธ</span>`,
      btnOk: '❌ ปฏิเสธ', btnCancel: 'ยกเลิก',
      onOk: () => {
        req.status = 'rejected';
        req.rejectedBy = CU.id;
        req.rejectedAt = nowStr();
        saveDB();
        notifyUser(req.requestedBy, '❌ คำขอถูกปฏิเสธ', `คำขอเพิ่มเครื่อง [${req.serial}] ${req.data.name} ถูกปฏิเสธโดย Admin`, null);
...
function showIncompleteList() {
  const incomplete = db.machines.filter(m => !getMachineEqStatus(m).isComplete);
  const total = db.machines.length;
  const incCount = incomplete.length;

  // update badge
  const btn = document.getElementById('incomplete-btn');
  if (btn) btn.style.display = incCount > 0 ? '' : 'none';

  const ov = document.createElement('div');
  ov.style.cssText = 'position:fixed;inset:0;z-index:9700;background:rgba(0,0,0,0.55);backdrop-filter:blur(4px);display:flex;align-items:flex-end;justify-content:center;padding:0';

  const sheet = document.createElement('div');
  sheet.style.cssText = 'background:white;border-radius:22px 22px 0 0;width:100%;max-width:600px;max-height:85vh;display:flex;flex-direction:column;animation:slideUp 0.25s ease';

  if (!incCount) {
    sheet.innerHTML = `
      <div style="padding:28px 24px;text-align:center">
        <div style="width:56px;height:56px;background:#f0fdf4;border-radius:16px;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-size:1.8rem">✅</div>
        <div style="font-size:1rem;font-weight:800;color:#0f172a;margin-bottom:6px">ข้อมูลครบทุกเครื่อง!</div>
        <div style="font-size:0.82rem;color:#64748b">${total} เครื่อง มีทั้ง Equipment No. และ Function Location</div>
        <button onclick="this.closest('[style*=inset]').remove()" style="margin-top:18px;padding:12px 24px;background:#f1f5f9;color:#64748b;border:none;border-radius:12px;font-size:0.88rem;font-weight:700;cursor:pointer;font-family:inherit">ปิด</button>
      </div>`;
  } else {
    const grouped = {};
    ...
    const rows = Object.entries(grouped).sort(([a],[b])=>a.localeCompare(b)).map(([dept, list]) => `
      <div style="margin-bottom:14px">
        <div style="font-size:0.68rem;font-weight:800;color:#94a3b8;text-transform:uppercase;letter-spacing:0.06em;margin-bottom:6px;padding:0 2px">${dept} (${list.length})</div>
        ${list.map(m => {
          const eqs = getMachineEqStatus(m);
          return `
          <div onclick="(function(el){const ov=el.closest('[style*=inset]');if(ov)ov.remove();setTimeout(()=>openMachineSheet('${m.id}'),200);})(this)" style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:white;border-radius:12px;margin-bottom:5px;cursor:pointer;border:1.5px solid #fde68a;transition:all 0.15s" onmousedown="this.style.background='#fffbeb'" onmouseup="this.style.background='white'">
            <div style="flex:1;min-width:0">
              <div style="font-size:0.82rem;font-weight:700;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${m.name}</div>
              <div style="font-family:'JetBrains Mono',monospace;font-size:0.62rem;color:#0369a1;font-weight:700;margin-top:1px">${m.serial}</div>
              <div style="display:flex;gap:4px;flex-wrap:wrap;margin-top:4px">
                ${!eqs.hasEQ?`<span style="background:#fff0f2;color:#c8102e;border:1px solid #fca5a5;border-radius:5px;padding:1px 6px;font-size:0.6rem;font-weight:700">❌ Equipment</span>`:''}
                ${!eqs.hasFL?`<span style="background:#fffbeb;color:#92400e;border:1px solid #fde68a;border-radius:5px;padding:1px 6px;font-size:0.6rem;font-weight:700">❌ Func Location</span>`:''}
              </div>
            </div>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#d97706" stroke-width="2.5" stroke-linecap="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
          </div>`;
        }).join('')}
      </div>`).join('');
    ...
    sheet.innerHTML = `
      ...
      <!-- Header -->
      <div style="padding:14px 18px 12px;flex-shrink:0;border-bottom:1px solid #f1f5f9">
        <div style="display:flex;align-items:center;justify-content:space-between">
          <div>
            <div style="font-size:0.95rem;font-weight:900;color:#0f172a;display:flex;align-items:center;gap:7px">
              <div style="width:28px;height:28px;background:linear-gradient(135deg,#d97706,#b45309);border-radius:8px;display:flex;align-items:center;justify-content:center">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.2" stroke-linecap="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
              </div>
              ข้อมูลแอร์ไม่ครบ
            </div>
            <div style="font-size:0.65rem;color:#94a3b8;margin-top:2px;padding-left:35px">
              <span style="color:#d97706;font-weight:800">${incCount}</span> จาก ${total} เครื่อง — ยังไม่มี Equipment No. หรือ Function Location
            </div>
          </div>
          <button onclick="this.closest('[style*=inset]').remove()" style="width:30px;height:30px;border-radius:50%;background:#f1f5f9;border:none;cursor:pointer;color:#64748b;font-size:0.85rem">✕</button>
        </div>
      </div>
      <!-- Body -->
      <div style="overflow-y:auto;flex:1;padding:14px 16px 24px;background:#f8fafc">
        ${rows}
      </div>`;
  }
