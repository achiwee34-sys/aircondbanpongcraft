        ...
        tickets.map(t => {
          ...
          return `<div onclick="ov.remove();setTimeout(()=>openDetail('${t.id}'),200)"
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:0.85rem;font-weight:800;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${t.problem}</div>
  ...
  const mkRow = (t, icon, color) => `
    <div onclick="openDetail('${t.id}')" style="display:flex;align-items:center;gap:9px;padding:9px 12px;border-radius:12px;background:#fafbff;border:1px solid #f1f5f9;margin-bottom:5px;cursor:pointer;transition:all 0.15s" onmousedown="this.style.background='#f0f4ff'" onmouseup="this.style.background='#fafbff'">
      ...
      <div style="flex:1;overflow:hidden">
        <div style="font-size:0.78rem;font-weight:700;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${t.problem||'—'}</div>
  ...
  box.innerHTML = msgs.map((m,i) => {
    ...
    if (isMe) return divider + `
      <div style="display:flex;justify-content:flex-end;margin-bottom:3px">
        <div style="max-width:75%">
          <div style="background:#c8102e;color:white;border-radius:18px 4px 18px 18px;padding:10px 14px;font-size:0.85rem;line-height:1.5;word-break:break-word;box-shadow:0 2px 8px rgba(200,16,46,0.25)">${m.text||''}</div>
    ...
    return divider + `
      <div style="display:flex;align-items:flex-end;gap:8px;margin-bottom:3px">
        ...
        <div style="max-width:75%">
          <div style="font-size:0.6rem;color:#64748b;margin-bottom:3px;font-weight:700">${name}</div>
...
function renderChatMessages() {
  ...
  msgs.forEach((m, idx) => {
    ...
    if (m.text) content += `<div style="font-size:0.88rem;line-height:1.5;word-break:break-word">${m.text}</div>`;

    // Name above first bubble (their messages)
    const nameLabel = !isMe && isFirstOfSender
      ? `<div style="font-size:0.62rem;font-weight:700;color:#c8102e;margin-bottom:3px;padding-left:2px">${m.name||''}</div>`
      : '';
  ...
  const wpHtml = wpTickets.length ? wpTickets.map(t => {
    ...
    return `<div style="background:white;border:1.5px solid rgba(230,81,0,0.25);border-radius:14px;padding:12px;margin-bottom:8px" onclick="openDetail('${t.id}')">
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:6px">
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;font-size:0.85rem;color:var(--text)">[${t.id}] ${t.problem}</div>
      ...
      <div style="padding:9px 10px 7px 14px">
        <div style="display:flex;align-items:center;gap:7px">
          <div style="flex:1;min-width:0">
            ...
            <div style="font-size:0.82rem;font-weight:800;color:#0f172a;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${t.problem}</div>
  ...
  const renderCard = t => {
    ...
    const infoBar = `
      <div style="padding:0 14px 10px;border-bottom:1px solid #f1f5f9">
        ...
        <div style="font-size:0.9rem;font-weight:800;color:#0f172a;line-height:1.35;margin-bottom:3px">${t.problem}</div>
    ...
    const reqBlock = reqRows.length ? `
      <div style="margin:10px 14px;background:#fafafa;border:1px solid #f1f5f9;border-radius:12px;overflow:hidden">
        ...
        ${reqRows.map((r,i)=>`
          <div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-bottom:1px solid #f8fafc">
            ...
            <div style="flex:1;min-width:0">
              ...
              ${r.note?`<div style="font-size:0.65rem;color:#9ca3af">${r.note}</div>`:''}
            </div>
            <div style="text-align:right;flex-shrink:0">
        ...
        ${req.note?`<div style="padding:7px 12px;background:#fffbeb;font-size:0.7rem;color:#78716c;border-top:1px solid #fde68a">📝 ${req.note}</div>`:''}
      </div>` : '';
    ...
    <div style="padding:12px 18px 14px;flex-shrink:0;border-bottom:1px solid #e2e8f0">
      <div style="display:flex;align-items:center;gap:10px">
        ...
        <div style="flex:1">
          ...
          <div style="font-size:0.68rem;color:#64748b;margin-top:1px">${t.id} · ${t.problem}</div>
        </div>
        ${editBtn}
      </div>
    </div>
    <div style="flex:1;overflow-y:auto;padding:14px 16px">
      ...
      ${rows.map((r,i)=>`
        <div style="background:white;border:1.5px solid #e2e8f0;border-radius:14px;margin-bottom:8px;overflow:hidden">
          ...
          ${r.note?`<div style="padding:7px 14px;background:#f8fafc;font-size:0.72rem;color:#64748b">${r.note}</div>`:''}
        </div>`).join('')}
    ...
    <div style="padding:12px 18px 14px;flex-shrink:0;border-bottom:1px solid #e2e8f0;background:#f8fafc">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">
        ...
        <div style="flex:1">
          ...
          <div style="font-size:0.68rem;color:#64748b;margin-top:1px">${tid} · ${t.problem}</div>
        </div>
...
function confirmPOReceived(tid) {
  const t = db.tickets.find(x=>x.id===tid);
  if (!t) return;
  const items = t.purchaseOrder?.rows?.filter(r=>r.name).map(r=>r.name).slice(0,3).join(', ') || 'อะไหล่';
  const total = t.purchaseOrder?.total || 0;
  const techName = db.users.find(u=>u.id===t.assigneeId)?.name || t.assignee || '—';
  // สร้าง confirm overlay
  document.querySelectorAll('.po-recv-overlay').forEach(e=>e.remove());
  const ov = document.createElement('div');
  ov.className = 'po-recv-overlay';
  ...
  ov.innerHTML = `
    <div style="background:white;border-radius:20px;padding:24px;max-width:320px;width:100%;box-shadow:0 20px 60px rgba(0,0,0,0.3);text-align:center">
      <div style="width:60px;height:60px;border-radius:50%;background:#f0fdf4;display:flex;align-items:center;justify-content:center;margin:0 auto 14px;font-size:1.8rem">📦</div>
      <div style="font-size:1rem;font-weight:800;color:#0f172a;margin-bottom:6px">ยืนยันรับอะไหล่?</div>
      <div style="font-size:0.78rem;color:#475569;margin-bottom:4px"><b>${tid}</b> · ${t.problem}</div>
      <div style="font-size:0.75rem;color:#64748b;background:#f8fafc;border-radius:10px;padding:8px 10px;margin-bottom:6px;text-align:left">
        <div>🔩 ${items}</div>
        <div style="margin-top:3px">💰 รวม <b style="color:#e65100">฿${Number(total).toLocaleString()}</b></div>
        <div style="margin-top:3px">👷 จะแจ้ง <b>${techName}</b> ให้ทราบ</div>
      </div>
      <div style="display:flex;gap:10px;margin-top:16px">
        <button id="porc-cancel" style="flex:1;padding:12px;border-radius:12px;border:1.5px solid #e5e7eb;background:white;font-size:0.88rem;font-weight:700;cursor:pointer;font-family:inherit">ยกเลิก</button>
        <button id="porc-ok" style="flex:1;padding:12px;border-radius:12px;border:none;background:#166534;color:white;font-size:0.88rem;font-weight:700;cursor:pointer;font-family:inherit">✅ ยืนยัน</button>
      </div>
    </div>`;
  document.body.appendChild(ov);
  ov.querySelector('#porc-cancel').onclick = () => ov.remove();
  ov.querySelector('#porc-ok').onclick = () => { ov.remove(); markPOReceived(tid); };
  ov.onclick = e => { if(e.target===ov) ov.remove(); };
}
function markPartsReceived(tid) { markPOReceived(tid); }

// อัปเดต PR หรือ PO ของแต่ละ row โดยตรงจาก card ติดตาม
function updateRowPRPO(tid, rowIdx, field, val) {
  const t = db.tickets.find(x=>x.id===tid); if(!t||!t.purchaseOrder) return;
  const rows = t.purchaseOrder.rows; if(!rows||!rows[rowIdx]) return;
...
function markPurchasingAndNotify(tid) {
  const t = db.tickets.find(x=>x.id===tid); if(!t) return;
  const now = nowStr();
  if (!t.purchaseOrder) return showToast('⚠️ กรุณากรอกรายการอะไหล่ก่อน');
  t.purchaseOrder.purchasing = true;
  t.purchaseOrder.purchasingAt = now;
  t.purchaseOrder.purchasingBy = CU.name;
  t.updatedAt = now;
  t.history = t.history||[];
  t.history.push({act:'🛒 ออก PR แล้ว กำลังสั่งซื้ออะไหล่',by:CU.name,at:now});
  const prList = [...new Set((t.purchaseOrder.rows||[]).map(r=>r.pr).filter(Boolean))].join(', ');
  const msg = 'ออก PR แล้ว'+(prList?' ('+prList+')':'')+' — กำลังดำเนินการสั่งซื้ออะไหล่';
  if (t.assigneeId) notifyUser(t.assigneeId, '🛒 ออก PR แล้ว ['+tid+']', msg, tid);
  if (t.reporterId) notifyUser(t.reporterId, '🛒 ออก PR แล้ว ['+tid+']', msg, tid);
  saveDB(); syncTicket(t);
