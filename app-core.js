...
if (localStorage.getItem(VER_KEY) !== APP_VER) {
  ...
  console.log('SCG.AIRCON: version updated to ' + APP_VER + ', resetting app data');
...
(function() {
  const DEFAULT_USERS = [
    {id:'u1',name:'ผู้ดูแลระบบ',username:'admin',password:'admin123',role:'admin',dept:'IT',tel:'',contact:''},
    {id:'u6',name:'นายสมหมาย ผู้จัดการ',username:'manager',password:'exec123',role:'executive',dept:'ฝ่ายบริหาร',tel:'',contact:''},
  ];
...
(function() {
  ...
  try {
    ...
    if (changed) {
      ...
      console.log('SCG.AIRCON: restored preserved data after version reset');
    }
...
if (db.machines) {
  ...
  if (migrated > 0) { localStorage.setItem(DB_KEY, JSON.stringify(db)); console.log(`Migrated ${migrated} machines: TPL → TPC`); }
}
...
async function loadMachinesData() {
  try {
    ...
    if (toAdd.length > 0) {
      ...
      console.log('[DB] Loaded ' + toAdd.length + ' machines from machines.json');
    }
  } catch(e) {
...
function initDB() {
  return {
    users: [
      // ── PATCH: password ถูก hash ด้วย SHA-256 ตอน first login อัตโนมัติ ──
      // admin123  → sha256 (migrate เมื่อ login ครั้งแรก)
      // exec123   → sha256 (migrate เมื่อ login ครั้งแรก)
      {id:'u1',name:'ผู้ดูแลระบบ',username:'admin',password:'admin123',role:'admin',dept:'IT',tel:'',contact:''},
      {id:'u6',name:'นายสมหมาย ผู้จัดการ',username:'manager',password:'exec123',role:'executive',dept:'ฝ่ายบริหาร',tel:'',contact:''},
    ],
    machines: [
...
function loadDB() {
  // โหลด db จาก localStorage (ใช้เมื่อต้องการ reload ข้อมูล)
  try {
  ...
  // migrate zone
  if (db.machines) {
    db.machines.forEach(m => { if (!m.zone) m.zone = 'process'; });
  }
  // migrate repairCost/partsCost สำหรับงานเก่า
  if (db.tickets) {
    db.tickets.forEach(t => {
      if (t.repairCost !== undefined) return;
      const poTotal = Number(t.purchaseOrder?.total || t.partsCost || 0);
      const total   = Number(t.cost || 0);
      if (total > 0 && poTotal > 0 && total > poTotal) {
        t.repairCost = total - poTotal;
        t.partsCost  = poTotal;
      } else if (total > 0 && poTotal === 0) {
        t.repairCost = total;
        t.partsCost  = 0;
      } else if (total > 0 && poTotal >= total) {
        t.repairCost = 0;
        t.partsCost  = poTotal;
      }
    });
...
async function migratePasswordIfNeeded(user, plainPass) {
  ...
  saveDB();
  console.log('[Auth] Password migrated → SHA-256 for:', user.username);
}
...
function selRole(r) { ['admin','tech','reporter','executive'].forEach(x => { const el=document.getElementById('rp-'+x); if(el) el.classList.toggle('active', x===r); }); }
...
async function doRegister() {  // PATCH: async เพื่อ await hashPassword
  ...
  // ถ้า Firebase พร้อม ส่งขึ้นก่อน ถ้าไม่พร้อมก็ยังมีใน localStorage
...
function fillD(u,p,r) { document.getElementById('lu').value=u; document.getElementById('lp').value=p; }
...
async function doLogin() {
  ...
  // ── PATCH: ค้นหา user จาก username ก่อน แล้วค่อย verify password ──
  ...
  if (!valid) {
    ...
    document.getElementById('lp').value = '';
    showLoginErr('Username หรือ Password ไม่ถูกต้อง');
    return;
  }
  clearLoginErr();
  ...
  CU = candidate;
  const sessionData = { uid: candidate.id, uname: candidate.username, exp: Date.now() + 90*24*60*60*1000 };
...
function tryAutoLogin() {
  try {
    const s = JSON.parse(localStorage.getItem('aircon_session')||'null');
    if (!s || Date.now() > s.exp) { localStorage.removeItem('aircon_session'); return false; }
    let found = db.users.find(x => x.id===s.uid && x.username===s.uname);
    if (!found) {
      // Firebase อาจยังไม่ sync — ลอง match แค่ uid หรือ username ก่อน
      found = db.users.find(x => x.id===s.uid) || db.users.find(x => x.username===s.uname);
    }
    if (!found) {
      // ยังหาไม่เจอ — รอ Firebase sync แล้วค่อย retry (max 5 วินาที)
      let retries = 0;
      const retry = setInterval(() => {
        retries++;
        const f = db.users.find(x => x.id===s.uid && x.username===s.uname)
                || db.users.find(x => x.id===s.uid)
                || db.users.find(x => x.username===s.uname);
        if (f) {
          clearInterval(retry);
          CU = f;
          s.exp = Date.now() + 90*24*60*60*1000;
          localStorage.setItem('aircon_session', JSON.stringify(s));
          document.getElementById('login-screen').style.display = 'none';
          try { document.getElementById('complete-sheet')?.close(); } catch(e){}
          document.getElementById('app').classList.add('visible');
          initApp();
        } else if (retries >= 10) {
          clearInterval(retry);
          // ล้าง session แล้วให้ login ใหม่
          localStorage.removeItem('aircon_session');
        }
      }, 500);
      return true; // บอกว่ากำลังรอ ไม่แสดง login screen ทันที
    }
    CU = found;
    // ต่ออายุ session อัตโนมัติ (sliding 90 วัน)
    s.exp = Date.now() + 90*24*60*60*1000;
    localStorage.setItem('aircon_session', JSON.stringify(s));
    document.getElementById('login-screen').style.display = 'none';
    try { document.getElementById('complete-sheet')?.close(); } catch(e){}
...
function openAcctEdit() { goPage('settings'); }
function previewAvatar(input) { spPreviewAvatar(input); }
function saveAcctProfile() { spSave(); }


// ============================================================
// PDF GENERATOR - ใบสรุปงานซ่อม
// ============================================================
function machGrid(rows) {
  let html = '<table style="width:100%;border-collapse:collapse;background:#eff6ff;border-radius:10px;overflow:hidden;margin-bottom:12px;table-layout:fixed">';
  rows.forEach(([label, val], i) => {
    const border = i < rows.length-1 ? 'border-bottom:1px solid #dbeafe' : '';
    const safeVal = String(val||'—');
    // ถ้ายาวเกิน 60 ตัว ตัดแสดงใน 2 บรรทัด
    const displayVal = safeVal.length > 80
      ? `<span style="word-break:break-all;overflow-wrap:anywhere">${safeVal}</span>`
      : safeVal;
    html += '<tr>'
      + `<td style="padding:6px 12px;${border};color:#1d4ed8;font-size:10px;font-weight:700;width:38%;vertical-align:top;white-space:nowrap">${label}</td>`
      + `<td style="padding:6px 12px;${border};font-weight:700;font-size:10.5px;color:#1e3a5f;word-break:break-word;overflow-wrap:anywhere">${displayVal}</td>`
      + '</tr>';
  });
  html += '</table>';
  return html;
...
<div class="page">
  ...
  // ── Bind DS globals ──
  window._dsField = (key,val) => { DS[key]=val; renderPreview(); };
...
function openPDFSettings(onSave, fromDesigner) {
  const cfg = Object.assign({
    theme:'red', logo:'', orgName:'SCG AIRCON',
    orgSub:'ใบสรุปงานซ่อมบำรุงเครื่องปรับอากาศ · BP Plant',
    footerText:'', footerPhone:'',
    watermark:'', watermarkOpacity:0.08,
    showSteps:true, showEquip:true, showPeople:true,
    showResult:true, showCost:true, showPhotos:true, showSig:true,
  }, getPDFConfig(), db.pdfConfig||{});

  const ov = document.createElement('div');
  ov.style.cssText = 'position:fixed;inset:0;z-index:9900;background:rgba(0,0,0,0.6);backdrop-filter:blur(8px);display:flex;align-items:flex-end;justify-content:center;animation:fadeIn 0.2s ease';
  const card = document.createElement('div');
  card.style.cssText = 'background:#f8fafc;border-radius:24px 24px 0 0;width:100%;max-width:600px;max-height:94vh;display:flex;flex-direction:column;animation:slideUp 0.28s cubic-bezier(0.32,0.72,0,1)';

  // ── Header ──
  const hdr = document.createElement('div');
  hdr.style.cssText = 'flex-shrink:0';
  hdr.innerHTML = '<div style="display:flex;justify-content:center;padding:10px 0 0"><div style="width:40px;height:4px;background:#d1d5db;border-radius:99px"></div></div>'
    + '<div style="padding:14px 18px 12px;border-bottom:1px solid #e5e7eb;display:flex;align-items:center;gap:12px">'
    + '<div style="width:40px;height:40px;background:linear-gradient(135deg,#0f172a,#1e293b);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:1.1rem;flex-shrink:0">📄</div>'
    + '<div style="flex:1"><div style="font-size:1rem;font-weight:900;color:#0f172a">'+(fromDesigner?'ออกแบบ PDF ใบสรุปงาน':'ตั้งค่า PDF')+'</div>'
    + '<div style="font-size:0.7rem;color:#64748b;margin-top:2px">ปรับแต่งแล้ว preview realtime</div></div>'
    + '<button id="pdfcfg-close" style="width:32px;height:32px;border-radius:50%;background:#f1f5f9;border:none;cursor:pointer;color:#64748b;font-size:1rem;flex-shrink:0">✕</button></div>';
  ...
  const secTitle = (t) => {
    ...
    d.style.cssText='font-size:0.68rem;font-weight:800;text-transform:uppercase;letter-spacing:.08em;color:#94a3b8;margin:16px 0 8px';
    d.textContent=t; return d;
  };
  // Helper: label+control row
  const row = (label, ctrl, sub) => {
    const d=document.createElement('div');
    d.style.cssText='display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:9px';
    const l=document.createElement('div');
    l.innerHTML='<div style="font-size:0.85rem;font-weight:600;color:#374151">'+label+'</div>'
      +(sub?'<div style="font-size:0.7rem;color:#94a3b8;margin-top:1px">'+sub+'</div>':'');
    d.appendChild(l); d.appendChild(ctrl); return d;
  };
  // Helper: text input
  const inp = (key, ph, type) => {
    const el=document.createElement('input');
    el.type=type||'text'; el.value=cfg[key]||''; el.placeholder=ph||'';
    el.style.cssText='padding:8px 11px;border:1.5px solid #e5e7eb;border-radius:9px;font-size:0.84rem;font-family:inherit;outline:none;width:200px;max-width:58%;color:#374151;background:white';
    el.oninput=()=>{cfg[key]=el.value;};
    el.onfocus=()=>el.style.borderColor='#c8102e';
    el.onblur=()=>el.style.borderColor='#e5e7eb';
    return el;
  };
  // Helper: toggle
  const toggle = (key) => {
    const btn=document.createElement('button');
    const upd=()=>{
      btn.style.background=cfg[key]?'#c8102e':'#e2e8f0';
      btn.innerHTML='<div style="width:20px;height:20px;border-radius:50%;background:white;box-shadow:0 1px 4px rgba(0,0,0,0.2);transition:transform 0.2s;transform:translateX('+(cfg[key]?'20':'0')+'px)"></div>';
    };
    btn.style.cssText='width:44px;height:24px;border-radius:99px;border:none;cursor:pointer;padding:2px;transition:background 0.2s;flex-shrink:0';
    upd();
    btn.onclick=()=>{cfg[key]=!cfg[key];upd();};
    return btn;
  };

  // ── THEME ──
  body.appendChild(secTitle('🎨 Theme สี'));
  const themeGrid=document.createElement('div');
  themeGrid.style.cssText='display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-bottom:4px';
  const themeMap=[['red','#c8102e','🔴 แดง'],['blue','#1d4ed8','🔵 น้ำเงิน'],['green','#16a34a','🟢 เขียว'],['black','#1a1a2e','⚫ ดำ']];
  themeMap.forEach(([key,cl,lbl])=>{
    const btn=document.createElement('button');
    const upd=()=>{btn.style.outline=cfg.theme===key?'3px solid '+cl:'3px solid transparent';btn.style.fontWeight=cfg.theme===key?'900':'700';};
    btn.style.cssText='padding:9px 4px;border-radius:10px;border:2px solid '+cl+';background:'+cl+'18;font-size:0.75rem;cursor:pointer;font-family:inherit;color:'+cl+';transition:all 0.15s;outline-offset:2px';
    btn.textContent=lbl; upd();
    btn.onclick=()=>{cfg.theme=key;themeGrid.querySelectorAll('button').forEach(b=>{b.style.outline='3px solid transparent';b.style.fontWeight='700'});upd();};
    themeGrid.appendChild(btn);
  });
  body.appendChild(themeGrid);

  // ── ORG ──
  body.appendChild(secTitle('🏢 ข้อมูลองค์กร'));
  body.appendChild(row('ชื่อองค์กร', inp('orgName','SCG AIRCON')));
  body.appendChild(row('คำอธิบาย header', inp('orgSub','ใบสรุปงานซ่อม...')));

  // ── LOGO ──
  body.appendChild(secTitle('🖼 โลโก้บริษัท'));
  const logoWrap=document.createElement('div');
  logoWrap.style.cssText='background:white;border:1.5px solid #e5e7eb;border-radius:12px;padding:12px;display:flex;align-items:center;gap:10px;margin-bottom:4px';
  const logoPreview=document.createElement('div');
  logoPreview.style.cssText='width:60px;height:40px;border:1px dashed #e5e7eb;border-radius:8px;display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;background:#f8fafc';
  if(cfg.logo){const im=document.createElement('img');im.src=cfg.logo;im.style.cssText='max-width:100%;max-height:100%;object-fit:contain';logoPreview.appendChild(im);}
  else{logoPreview.innerHTML='<span style="font-size:1.2rem">🏢</span>';}
  const logoRight=document.createElement('div');logoRight.style.flex='1';
  const logoUpBtn=document.createElement('button');
  logoUpBtn.style.cssText='padding:7px 12px;background:#f1f5f9;border:1.5px solid #e5e7eb;border-radius:8px;font-size:0.78rem;font-weight:700;cursor:pointer;font-family:inherit;color:#374151;margin-right:6px';
  logoUpBtn.textContent=cfg.logo?'🔄 เปลี่ยน':'📤 อัพโหลด';
  const logoFile=document.createElement('input');logoFile.type='file';logoFile.accept='image/*';logoFile.style.display='none';
  logoFile.onchange=(e)=>{
    const file=e.target.files[0];if(!file)return;
    const reader=new FileReader();
    reader.onload=(ev)=>{
      cfg.logo=ev.target.result;
      logoPreview.innerHTML='';const im=document.createElement('img');im.src=cfg.logo;im.style.cssText='max-width:100%;max-height:100%;object-fit:contain';logoPreview.appendChild(im);
      logoUpBtn.textContent='🔄 เปลี่ยน';if(logoClear)logoClear.style.display='';
    };reader.readAsDataURL(file);
  };
  logoUpBtn.onclick=()=>logoFile.click();
  const logoClear=document.createElement('button');
  logoClear.style.cssText='padding:7px 10px;background:none;border:none;font-size:0.75rem;font-weight:700;cursor:pointer;font-family:inherit;color:#c8102e;display:'+(cfg.logo?'':'none');
  logoClear.textContent='✕ ลบ';
  logoClear.onclick=()=>{cfg.logo='';logoPreview.innerHTML='<span style="font-size:1.2rem">🏢</span>';logoUpBtn.textContent='📤 อัพโหลด';logoClear.style.display='none';};
  logoRight.appendChild(logoUpBtn);logoRight.appendChild(logoClear);logoRight.appendChild(logoFile);
  logoWrap.appendChild(logoPreview);logoWrap.appendChild(logoRight);
  body.appendChild(logoWrap);

  // ── รูปข้างใบเสนอราคา ──
  body.appendChild(secTitle('🖼 รูปข้างใบเสนอราคา'));
  const qiWrap=document.createElement('div');
  qiWrap.style.cssText='background:white;border:1.5px solid #e5e7eb;border-radius:12px;padding:12px;display:flex;align-items:center;gap:10px;margin-bottom:4px';
  const qiPreview=document.createElement('div');
  qiPreview.style.cssText='width:60px;height:40px;border:1px dashed #e5e7eb;border-radius:8px;display:flex;align-items:center;justify-content:center;overflow:hidden;flex-shrink:0;background:#f8fafc';
  if(cfg.quotationImg){const im=document.createElement('img');im.src=cfg.quotationImg;im.style.cssText='max-width:100%;max-height:100%;object-fit:contain';qiPreview.appendChild(im);}
  else{qiPreview.innerHTML='<span style="font-size:1.2rem">📄</span>';}
  const qiRight=document.createElement('div');qiRight.style.flex='1';
  const qiUpBtn=document.createElement('button');
  qiUpBtn.style.cssText='padding:7px 12px;background:#f1f5f9;border:1.5px solid #e5e7eb;border-radius:8px;font-size:0.78rem;font-weight:700;cursor:pointer;font-family:inherit;color:#374151;margin-right:6px';
  qiUpBtn.textContent=cfg.quotationImg?'🔄 เปลี่ยน':'📤 อัพโหลด';
  const qiFile=document.createElement('input');qiFile.type='file';qiFile.accept='image/*';qiFile.style.display='none';
  qiFile.onchange=(e)=>{
    const file=e.target.files[0];if(!file)return;
    const reader=new FileReader();
    reader.onload=(ev)=>{
      cfg.quotationImg=ev.target.result;
      qiPreview.innerHTML='';const im=document.createElement('img');im.src=cfg.quotationImg;im.style.cssText='max-width:100%;max-height:100%;object-fit:contain';qiPreview.appendChild(im);
      qiUpBtn.textContent='🔄 เปลี่ยน';if(qiClear)qiClear.style.display='';
    };reader.readAsDataURL(file);
  };
  qiUpBtn.onclick=()=>qiFile.click();
  const qiClear=document.createElement('button');
  qiClear.style.cssText='padding:7px 10px;background:none;border:none;font-size:0.75rem;font-weight:700;cursor:pointer;font-family:inherit;color:#c8102e;display:'+(cfg.quotationImg?'':'none');
  qiClear.textContent='✕ ลบ';
  qiClear.onclick=()=>{cfg.quotationImg='';qiPreview.innerHTML='<span style="font-size:1.2rem">📄</span>';qiUpBtn.textContent='📤 อัพโหลด';qiClear.style.display='none';};
  qiRight.appendChild(qiUpBtn);qiRight.appendChild(qiClear);qiRight.appendChild(qiFile);
  qiWrap.appendChild(qiPreview);qiWrap.appendChild(qiRight);
  body.appendChild(qiWrap);

  // ── FOOTER ──
  body.appendChild(secTitle('📋 Footer'));
  body.appendChild(row('ข้อความ footer', inp('footerText','เช่น ฝ่ายวิศวกรรม SCG')));
  body.appendChild(row('เบอร์โทรติดต่อ', inp('footerPhone','081-xxx-xxxx')));

  // ── WATERMARK ──
  body.appendChild(secTitle('💧 ลายน้ำ'));
  body.appendChild(row('ข้อความ', inp('watermark','เช่น DRAFT'), 'แสดงตรงกลางใบ'));
  const opRow=document.createElement('div');
  opRow.style.cssText='display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:9px';
  opRow.innerHTML='<div style="font-size:0.85rem;font-weight:600;color:#374151">ความเข้ม</div>';
  const opSlider=document.createElement('input');
  opSlider.type='range';opSlider.min=0.02;opSlider.max=0.25;opSlider.step=0.01;
  opSlider.value=cfg.watermarkOpacity||0.08;
  opSlider.style.cssText='width:130px;accent-color:#c8102e';
  opSlider.oninput=()=>{cfg.watermarkOpacity=parseFloat(opSlider.value);};
  opRow.appendChild(opSlider);body.appendChild(opRow);

  // ── SECTIONS ──
  body.appendChild(secTitle('📑 Section ที่แสดง'));
  const sectionsWrap=document.createElement('div');
  sectionsWrap.style.cssText='background:white;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden';
  const sections=[
    ['showSteps','Progress Steps','ขั้นตอนงานแจ้ง→ปิด'],
    ['showPeople','ผู้เกี่ยวข้อง','ผู้แจ้ง + ช่างซ่อม'],
    ['showEquip','ข้อมูลอุปกรณ์','Equipment No., BTU ฯลฯ'],
    ['showResult','ผลการซ่อม','สรุปงาน + อะไหล่'],
    ['showCost','ค่าใช้จ่าย','แสดงเฉพาะถ้ามีค่า'],
    ['showPhotos','รูปก่อน/หลัง','ภาพประกอบการซ่อม'],
    ['showSig','ลายเซ็นรับรอง','ช่าง + ผู้แจ้ง + Admin'],
  ];
  sections.forEach(([key,lbl,sub],i)=>{
    const item=document.createElement('div');
    item.style.cssText='display:flex;align-items:center;justify-content:space-between;gap:10px;padding:11px 14px;'+(i<sections.length-1?'border-bottom:1px solid #f1f5f9':'');
    item.innerHTML='<div><div style="font-size:0.85rem;font-weight:600;color:#374151">'+lbl+'</div><div style="font-size:0.7rem;color:#94a3b8;margin-top:1px">'+sub+'</div></div>';
    item.appendChild(toggle(key));
  ...
  foot.style.cssText='padding:10px 16px 22px;flex-shrink:0;border-top:1px solid #e5e7eb;background:#f8fafc;display:flex;gap:8px';

  const previewBtn=document.createElement('button');
  previewBtn.style.cssText='flex:1;padding:14px;background:white;border:1.5px solid #e5e7eb;color:#374151;border-radius:14px;font-size:0.88rem;font-weight:700;cursor:pointer;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:6px';
  previewBtn.innerHTML='<span>👁</span> Preview';
  previewBtn.onclick=()=>{
    savePDFConfig(cfg);
    // หา ticket ที่ดีที่สุดสำหรับ preview
    const sample = db.tickets.find(t=>['done','verified','closed'].includes(t.status))
      || db.tickets.find(t=>t.summary)
      || db.tickets.slice(-1)[0]
      || db.tickets[0];
    if(sample){
      ov.remove(); // ลบ designer sheet ก่อน
      setTimeout(()=>generateRepairPDF(sample.id), 100);
    } else {
      showToast('⚠️ ยังไม่มีใบงาน — กรุณาสร้างใบงานก่อน');
    }
  };

  const saveBtn=document.createElement('button');
  saveBtn.style.cssText='flex:2;padding:14px;background:linear-gradient(135deg,#0f172a,#1e293b);color:white;border:none;border-radius:14px;font-size:0.92rem;font-weight:800;cursor:pointer;font-family:inherit;display:flex;align-items:center;justify-content:center;gap:6px;box-shadow:0 4px 16px rgba(0,0,0,0.2)';
  saveBtn.innerHTML='<span>💾</span> บันทึกใช้งาน';
  saveBtn.onclick=()=>{
    savePDFConfig(cfg);
    showToast('✅ บันทึกรูปแบบ PDF แล้ว');
    ov.remove();
    if(onSave) onSave();
  };

  foot.appendChild(previewBtn);foot.appendChild(saveBtn);
  card.appendChild(foot);

  card.querySelector('#pdfcfg-close').onclick=()=>ov.remove();
...
function forceReset() {
  if (!confirm('ล้างข้อมูลทั้งหมดและโหลดแอพใหม่?\nTicket และผู้ใช้ที่เพิ่มเองจะหาย')) return;
  localStorage.clear();
  location.reload();
}
function resetApp() {
...
function migrateOldSummaryFormat() {
  ...
  if (changed) {
    saveDB();
    console.log('Summary migration done');
  }
...
function closeMoreMenu() {
  ...
  document.getElementById('more-menu-btn')?.classList.remove('active');
}
...
function goPage(name) {
  ...
  requestAnimationFrame(() => {
    ...
    else if (name === 'new') { populateMachineSelect(); setTimeout(()=>{ setPriority('mid'); const priField=document.getElementById('nt-priority-field'); if(priField) priField.style.display=CU.role==='reporter'?'none':''; },50); }
