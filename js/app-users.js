...
function importMachines(){if(!xlData.length)return;var gc=function(k){var el=document.getElementById('xlmap-'+k);return el?el.value:'';};var added=0,skipped=0;xlData.forEach(function(row){var name=row[gc('name')];if(!name)return;var serial=String(row[gc('serial')]||'').trim();if(serial&&db.machines.find(function(m){return String(m.serial||'').trim()===serial||(m.id&&m.id.replace(/^csv_/,'').trim()===serial);})){skipped++;return;}var brand=String(row[gc('brandCDU')]||row[gc('brand')]||'').trim();db.machines.push({id:'M'+Date.now()+Math.random().toString(36).slice(2,5),name:String(name).trim(),serial:serial,dept:String(row[gc('dept')]||'').trim(),room:String(row[gc('room')]||'').trim(),brand:brand,brandCDU:brand,modelCDU:String(row[gc('modelCDU')]||'').trim(),brandFCU:String(row[gc('brandFCU')]||'').trim(),modelFCU:String(row[gc('modelFCU')]||'').trim(),btu:String(row[gc('btu')]||'').trim(),refrigerant:String(row[gc('refrigerant')]||'').trim(),vendor:String(row[gc('vendor')]||'').trim(),funcLoc:String(row[gc('funcLoc')]||'').trim(),equipNo:String(row[gc('equipNo')]||'').trim(),pmMonths:String(row[gc('pmMonths')]||'').trim(),installDate:String(row[gc('installDate')]||'').trim(),nextPM:String(row[gc('nextPM')]||'').trim(),note:String(row[gc('note')]||'').trim(),status:'normal',createdAt:nowStr()});added++;});if(added>0){saveDB();if(typeof renderMachines==='function')renderMachines();}closeSheet('import');resetExcel();showToast(added>0?'✅ นำเข้า '+added+' เครื่องแล้ว'+(skipped>0?' (ข้าม '+skipped+' ซ้ำ)':''):'⚠️ ไม่มีข้อมูลใหม่ (อาจซ้ำทั้งหมด)');}
