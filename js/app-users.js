...
function renderColMap(){var el=document.getElementById('xl-col-map');if(!el)return;var fields=[{key:'name',label:'ชื่อเครื่อง *',hints:['ชื่อเครื่อง','name']},{key:'serial',label:'Air ID / Serial',hints:['air id','serial','รหัส']},{key:'dept',label:'แผนก',hints:['แผนก','dept']},{key:'funcLoc',label:'Func.Loc',hints:['func.loc','func loc','funcloc']},{key:'equipNo',label:'Equipment No.',hints:['equipment','equip']},{key:'brandCDU',label:'ยี่ห้อ CDU',hints:['ยี่ห้อ cdu']},{key:'modelCDU',label:'รุ่น CDU',hints:['รุ่น cdu']},{key:'brandFCU',label:'ยี่ห้อ FCU',hints:['ยี่ห้อ fcu']},{key:'modelFCU',label:'รุ่น FCU',hints:['รุ่น fcu']},{key:'btu',label:'BTU',hints:['btu']},{key:'refrigerant',label:'น้ำยา',hints:['น้ำยา','refrigerant']},{key:'vendor',label:'Vendor',hints:['vendor']},{key:'pmMonths',label:'PM ทุก (เดือน)',hints:['pm ทุก']},{key:'installDate',label:'วันติดตั้ง',hints:['ติดตั้ง','install']},{key:'nextPM',label:'วันที่ PM ครั้งถัดไป',hints:['pm ครั้งถัดไป','next pm']},{key:'room',label:'ห้อง',hints:['room','ห้อง']},{key:'note',label:'หมายเหตุ',hints:['note','หมายเหตุ']}];el.innerHTML=fields.map(function(f){var auto='';for(var hi=0;hi<(f.hints||[]).length;hi++){var hint=f.hints[hi];auto=xlHeaders.find(function(h){return h.toLowerCase().includes(hint);})||'';if(auto)break;}return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><label style="width:120px;font-size:0.72rem;font-weight:700;flex-shrink:0">'+f.label+'</label><select id="xlmap-'+f.key+'" style="flex:1;padding:6px 8px;border:1.5px solid var(--border);border-radius:8px;font-size:0.72rem;font-family:inherit"><option value="">(ไม่นำเข้า)</option>'+xlHeaders.map(function(h){return '<option value="'+h+'"'+(h===auto?' selected':'')+'>'+h+'</option>';}).join('')+'</select></div>';}).join('');}
function importMachines(){if(!xlData.length)return;var gc=function(k){var el=document.getElementById('xlmap-'+k);return el?el.value:'';};var added=0,skipped=0;xlData.forEach(function(row){var name=row[gc('name')];if(!name)return;var serial=String(row[gc('serial')]||'').trim();if(serial&&db.machines.find(function(m){return String(m.serial||'').trim()===serial||(m.id&&m.id.replace(/^csv_/,'').trim()===serial);})){skipped++;return;}var newId=serial?'csv_'+serial:('csv_M'+Date.now()+Math.random().toString(36).slice(2,5));if(db.machines.find(function(m){return m.id===newId;})){skipped++;return;}var brand=String(row[gc('brandCDU')]||row[gc('brand')]||'').trim();var nm={id:newId,name:String(name).trim(),serial:serial,dept:String(row[gc('dept')]||'').trim(),location:String(row[gc('room')]||'').trim(),room:String(row[gc('room')]||'').trim(),brand:brand,brandCDU:brand,mfrCDU:brand,modelCDU:String(row[gc('modelCDU')]||'').trim(),brandFCU:String(row[gc('brandFCU')]||'').trim(),mfrFCU:String(row[gc('brandFCU')]||'').trim(),modelFCU:String(row[gc('modelFCU')]||'').trim(),btu:String(row[gc('btu')]||'').trim(),refrigerant:String(row[gc('refrigerant')]||'').trim(),vendor:String(row[gc('vendor')]||'').trim(),funcLoc:String(row[gc('funcLoc')]||'').trim(),equipment:String(row[gc('equipNo')]||'').trim(),interval:parseInt(row[gc('pmMonths')]||6)||6,install:String(row[gc('installDate')]||'').trim(),note:String(row[gc('note')]||'').trim(),status:'normal',addedAt:new Date().toISOString(),addedBy:(typeof CU!=='undefined'&&CU)?CU.id:'',createdAt:nowStr()};db.machines.push(nm);if(typeof syncMachine==='function')syncMachine(nm);added++;});if(added>0){saveDB();if(typeof fsSaveNow==='function')fsSaveNow();if(typeof renderMachines==='function')renderMachines();if(typeof populateMachineSelect==='function')populateMachineSelect();}closeSheet('import');resetExcel();showToast(added>0?'✅ นำเข้า '+added+' เครื่องแล้ว'+(skipped>0?' (ข้าม '+skipped+' ซ้ำ)':''):'⚠️ ไม่มีข้อมูลใหม่ (อาจซ้ำทั้งหมด)');}
