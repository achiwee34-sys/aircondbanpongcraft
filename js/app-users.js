...
var m=db.machines.find(function(x){return x.id===mid||(x.id||'').toUpperCase()===cleanId||(x.serial||'').toUpperCase()===cleanId||(x.serial||'').toUpperCase().includes(cleanId)&&cleanId.length>=4;});
if(m){var ds=document.getElementById('nt-dept');if(ds){[].forEach.call(ds.options,function(o){if(o.value===m.dept)o.selected=true;});// update custom picker display
// ── BUG FIX #2: ส่ง m.id ใน else branch ด้วย — ป้องกันไม่ auto-select เครื่อง ──
if(typeof selectDeptPickerItem==='function'&&m.dept){var palette=['#c8102e','#e65100','#0369a1','#059669','#7c3aed','#d97706'];var idx=(_deptPickerDepts||[]).indexOf(m.dept);var col=palette[idx>=0?idx%palette.length:0];selectDeptPickerItem(m.dept,col);}else{onDeptChange(m.dept,m.id);}setTimeout(function(){if(typeof onDeptChange==='function')onDeptChange(m.dept,m.id);},200);}showToast('✅ พบเครื่อง: '+m.name);}else showToast('❌ ไม่พบเครื่องรหัส: '+mid);}
...
function readExcel(input){var file=input.files[0];if(!file)return;var reader=new FileReader();reader.onload=function(e){var wb=XLSX.read(new Uint8Array(e.target.result),{type:'array'});var ws=wb.Sheets[wb.SheetNames[0]];xlData=XLSX.utils.sheet_to_json(ws,{defval:''});xlHeaders=xlData.length>0?Object.keys(xlData[0]):[];document.getElementById('xl-map').style.display='block';document.getElementById('xl-import-btn').style.display='';renderColMap();var _cntEl=document.getElementById('xl-count');if(_cntEl)_cntEl.textContent='พบ '+xlData.length+' แถวข้อมูลในไฟล์';};reader.readAsArrayBuffer(file);}
var IMPORT_FIELDS=[{key:'name',label:'ชื่อเครื่อง *',hints:['ชื่อเครื่อง','name']},{key:'serial',label:'Air ID / Serial',hints:['air id','serial','รหัส']},{key:'dept',label:'แผนก',hints:['แผนก','dept']},{key:'funcLoc',label:'Func.Loc',hints:['func.loc','func loc','funcloc']},{key:'equipNo',label:'Equipment No.',hints:['equipment','equip']},{key:'brandCDU',label:'ยี่ห้อ CDU',hints:['ยี่ห้อ cdu']},{key:'modelCDU',label:'รุ่น CDU',hints:['รุ่น cdu']},{key:'brandFCU',label:'ยี่ห้อ FCU',hints:['ยี่ห้อ fcu']},{key:'modelFCU',label:'รุ่น FCU',hints:['รุ่น fcu']},{key:'btu',label:'BTU',hints:['btu']},{key:'refrigerant',label:'น้ำยา',hints:['น้ำยา','refrigerant']},{key:'vendor',label:'Vendor',hints:['vendor']},{key:'pmMonths',label:'PM ทุก (เดือน)',hints:['pm ทุก']},{key:'installDate',label:'วันติดตั้ง',hints:['ติดตั้ง','install']},{key:'room',label:'ห้อง',hints:['ห้อง',' room']},{key:'note',label:'หมายเหตุ',hints:['note','หมายเหตุ']}];
// หา auto-map ให้แต่ละ field — 1 คอลัมน์ auto-map ได้แค่ field เดียว
// (แก้บั๊ก: ไฟล์ export ของระบบมีหัวข้อ "ชื่อเครื่อง/ห้อง" คอลัมน์เดียว เดิมทั้ง name และ room
//  จะ auto-select คอลัมน์เดียวกัน ทำให้ location/room ถูกเติมข้อมูลซ้ำกับชื่อเครื่องโดยไม่ตั้งใจ)
function _autoMapColumns(fields,headers){var used={};var map={};fields.forEach(function(f){var auto='';for(var hi=0;hi<(f.hints||[]).length;hi++){var hint=f.hints[hi];var cand=headers.find(function(h){return !used[h]&&h.toLowerCase().includes(hint);});if(cand){auto=cand;break;}}if(auto){used[auto]=true;}map[f.key]=auto;});return map;}
function renderColMap(){var el=document.getElementById('col-map-box');if(!el)return;var auto=_autoMapColumns(IMPORT_FIELDS,xlHeaders);el.innerHTML=IMPORT_FIELDS.map(function(f){var a=auto[f.key]||'';return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><label style="width:120px;font-size:0.72rem;font-weight:700;flex-shrink:0">'+f.label+'</label><select id="xlmap-'+f.key+'" style="flex:1;padding:6px 8px;border:1.5px solid var(--border);border-radius:8px;font-size:0.72rem;font-family:inherit"><option value="">(ไม่นำเข้า)</option>'+xlHeaders.map(function(h){return '<option value="'+h+'"'+(h===a?' selected':'')+'>'+h+'</option>';}).join('')+'</select></div>';}).join('');}
function importMachines(){if(!xlData.length)return;var gc=function(k){var el=document.getElementById('xlmap-'+k);return el?el.value:'';};var added=0,skippedDupSerial=0,skippedDupEquip=0,skippedNoName=0,warnDupEquip=0;var seenEquip={};(db.machines||[]).forEach(function(m){if(m.equipment)seenEquip[m.equipment.trim().toLowerCase()]=true;});
var _cleanNum=function(v){var s=String(v==null?'':v).replace(/[,\s]/g,'');var n=parseFloat(s);return isNaN(n)?'':String(n);};
xlData.forEach(function(row){
  var name=row[gc('name')];
  if(!name||!String(name).trim()){skippedNoName++;return;}
  var serial=String(row[gc('serial')]||'').trim();
  if(serial&&db.machines.find(function(m){return String(m.serial||'').trim()===serial||(m.id&&m.id.replace(/^csv_/,'').trim()===serial);})){skippedDupSerial++;return;}
  var newId=serial?'csv_'+serial:('csv_M'+Date.now()+Math.random().toString(36).slice(2,5));
  if(db.machines.find(function(m){return m.id===newId;})){skippedDupSerial++;return;}
  var equipNo=String(row[gc('equipNo')]||'').trim();
  var equipKey=equipNo.toLowerCase();
  // Equipment No. ซ้ำ (คนละเครื่องแต่ใช้เลขครุภัณฑ์เดียวกัน) — ไม่บล็อกการนำเข้า แต่เตือนให้ตรวจสอบภายหลัง
  var isDupEquip=false;
  if(equipNo&&seenEquip[equipKey]){warnDupEquip++;isDupEquip=true;} else if(equipNo){seenEquip[equipKey]=true;}
  var brand=String(row[gc('brandCDU')]||row[gc('brand')]||'').trim();
  var nm={id:newId,name:String(name).trim(),serial:serial,dept:String(row[gc('dept')]||'').trim(),location:String(row[gc('room')]||'').trim(),room:String(row[gc('room')]||'').trim(),brand:brand,brandCDU:brand,mfrCDU:brand,modelCDU:String(row[gc('modelCDU')]||'').trim(),brandFCU:String(row[gc('brandFCU')]||'').trim(),mfrFCU:String(row[gc('brandFCU')]||'').trim(),modelFCU:String(row[gc('modelFCU')]||'').trim(),btu:_cleanNum(row[gc('btu')]),refrigerant:String(row[gc('refrigerant')]||'').trim(),vendor:String(row[gc('vendor')]||'').trim(),funcLoc:String(row[gc('funcLoc')]||'').trim(),equipment:equipNo,interval:parseInt(row[gc('pmMonths')]||6)||6,install:String(row[gc('installDate')]||'').trim(),note:String(row[gc('note')]||'').trim(),status:'normal',addedAt:new Date().toISOString(),addedBy:(typeof CU!=='undefined'&&CU)?CU.id:'',createdAt:nowStr(),dupEquipFlag:isDupEquip?true:undefined};
  db.machines.push(nm);
  if(typeof syncMachine==='function')syncMachine(nm);
  added++;
});
if(added>0){saveDB();if(typeof fsSaveNow==='function')fsSaveNow();if(typeof renderMachines==='function')renderMachines();if(typeof populateMachineSelect==='function')populateMachineSelect();}
closeSheet('import');resetExcel();
var msgParts=[];
if(added>0)msgParts.push('✅ นำเข้า '+added+' เครื่องแล้ว');
var skipBits=[];
if(skippedDupSerial>0)skipBits.push('Serial ซ้ำ '+skippedDupSerial);
if(skippedNoName>0)skipBits.push('ไม่มีชื่อเครื่อง '+skippedNoName);
if(skipBits.length)msgParts.push('(ข้าม: '+skipBits.join(', ')+')');
if(warnDupEquip>0)msgParts.push('⚠️ พบ Equipment No. ซ้ำกัน '+warnDupEquip+' รายการ — นำเข้าแล้วแต่ควรตรวจสอบ');
showToast(msgParts.length?msgParts.join(' '):'⚠️ ไม่มีข้อมูลใหม่ (อาจซ้ำทั้งหมด)');
}
