...
function renderColMap(){var el=document.getElementById('xl-col-map');if(!el)return;var auto=_autoMapColumns(IMPORT_FIELDS,xlHeaders);el.innerHTML=IMPORT_FIELDS.map(function(f){var a=auto[f.key]||'';return '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px"><label style="width:120px;font-size:0.72rem;font-weight:700;flex-shrink:0">'+f.label+'</label><select id="xlmap-'+f.key+'" style="flex:1;padding:6px 8px;border:1.5px solid var(--border);border-radius:8px;font-size:0.72rem;font-family:inherit"><option value="">(ไม่นำเข้า)</option>'+xlHeaders.map(function(h){return '<option value="'+h+'"'+(h===a?' selected':'')+'>'+h+'</option>';}).join('')+'</select></div>';}).join('');}
