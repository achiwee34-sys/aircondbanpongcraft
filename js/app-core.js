...
// ── Responsive overlay page position (v33 — pure CSS vars) ──────────
function _overlayStyle(zIndex, bg, anim) {
  // ใช้ CSS variables แทน JS getBoundingClientRect() ป้องกัน layout ยังไม่พร้อม
  // --overlay-left / --overlay-bottom ถูก set โดย @media rules ใน main.css
  const top    = `calc(var(--head-h,56px) + var(--safe-top,env(safe-area-inset-top,0px)))`;
  const bottom = `var(--overlay-bottom,calc(var(--nav-h,56px) + var(--safe-bot,env(safe-area-inset-bottom,0px))))`;
  const left   = `var(--overlay-left,0px)`;
...
try {
  ...
  window.db = db; // expose เป็น global สำหรับ line-messaging.js และ script อื่น
} catch(e) {
...
async function doLogin() {
  ...
  try {
    ...
    if (!valid) {
      ...
      document.getElementById('lp').value = '';
      _loginAttempts = (_loginAttempts||0) + 1; // legacy counter (unused — real lockout uses _getLockoutState below)
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (typeof autoLiffLogin === 'function') {
    // defer ให้ db + Firebase load ก่อน และให้ LIFF SDK มีเวลาโหลด
    setTimeout(autoLiffLogin, 500);
  }
...
function goPage(name) {
  ...
  document.querySelectorAll('.cdel-overlay, #admin-manage-tk-ov').forEach(el => el.remove());
  // ── cleanup pur-po-panel เมื่อออกจากหน้า purchase ──
  if (_activePage === 'purchase' && name !== 'purchase') {
    if (typeof closePOInline === 'function') closePOInline();
  }
  ...
  requestAnimationFrame(() => {
    if (name === 'home') renderHome();
      ...
      // ── reset pur-po-panel ก่อนเสมอ ป้องกันหน้าซ้อนเวลากลับมา ──
      const _ppReset = document.getElementById('pur-po-panel');
      const _lpReset = document.getElementById('pur-list-panel');
      if (_ppReset) {
        // cleanup keyboard listener
        if (_ppReset._kbFix && window.visualViewport) {
          window.visualViewport.removeEventListener('resize', _ppReset._kbFix);
