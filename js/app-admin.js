...
async function manualLoad() {
  ...
  try {
    ...
    if (typeof refreshPage === "function") refreshPage(); else location.reload(); if(typeof updateOpenBadge==="function") updateOpenBadge(); if(typeof updateNBadge==="function") updateNBadge();
...
async function clearFirestoreData(type) {
  ...
  try {
    ...
    if (_firebaseReady && FSdb) {
      // BUG FIX (Bug 1): รอ Firebase auth พร้อมก่อน write
      // PC admin ใช้ signInAnonymously ซึ่งอาจยังไม่เสร็จตอนกดปุ่มล้างข้อมูล
      // ถ้า auth ยังไม่พร้อม → ลอง signInAnonymously ใหม่แล้วรอ
      if (typeof _waitForAuth === 'function') {
        const authed = await _waitForAuth(5000);
        if (!authed && firebase && firebase.auth) {
          try {
            await firebase.auth().signInAnonymously();
            await _waitForAuth(5000);
          } catch(authErr) {
            console.warn('[clearFirestoreData] auth retry failed:', authErr.message);
          }
        }
      }
