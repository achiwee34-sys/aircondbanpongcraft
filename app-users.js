...
if(typeof selectDeptPickerItem==='function'&&m.dept){var palette=['#c8102e','#e65100','#0369a1','#059669','#7c3aed','#d97706'];var idx=(_deptPickerDepts||[]).indexOf(m.dept);var col=palette[idx>=0?idx%palette.length:0];selectDeptPickerItem(m.dept,col);}else{onDeptChange(m.dept);}setTimeout(function(){if(typeof onDeptChange==='function')onDeptChange(m.dept,m.id);},200);}showToast('✅ พบเครื่อง: '+m.name);}else showToast('❌ ไม่พบเครื่องรหัส: '+mid);}
