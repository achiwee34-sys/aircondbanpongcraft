...
async function submitTicket() { // PATCH v67
  ...
  if (existingJobs.length > 0) {
    ...
    showAlert({
      ...
      onOk: async () => { await _doSubmitTicket(mid, prob); }, // PATCH v67
      onCancel: () => {
        goPage('tickets');
...
async function _doSubmitTicket(mid, prob) {
  ...
  const _seq = db._seq++;
  const tid = 'TK'+_mm+_yy+String(_seq).padStart(3,'0');
  const now = nowStr();
  ...
  showAlert({
    ...
    msg: `เลขที่ใบงาน <strong style="color:#0f172a;font-family:monospace">${tid}</strong><br>
      <span style="font-size:0.82rem;color:#64748b">${prob} · ${m.name}</span><br>
    ...
    <div style="background:linear-gradient(135deg,#e65100,#c2410c);padding:18px 20px 16px;position:relative;overflow:hidden">
      ...
      <div style="display:flex;align-items:center;gap:12px;position:relative">
        ...
        <div>
          ...
          <div style="font-size:0.7rem;color:rgba(255,255,255,0.75);margin-top:2px">${t.id} · ${t.problem}</div>
        </div>
      </div>
    </div>
  ...
  if (lines.length > 1) {
    return lines.map(function(l){
      return '<div style="display:flex;gap:6px;align-items:flex-start;margin-bottom:4px">'
        ...
        + '<span style="flex:1">' + l.replace(/^[-–\s]+/,'').trim() + '</span></div>';
    }).join('');
  }
  return '<div>' + text + '</div>';
}
    ...
    strip.innerHTML = `
      <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:8px;margin-bottom:7px">
        <div style="flex:1;min-width:0">
          ...
          <div style="font-size:0.88rem;font-weight:900;color:#0f172a;line-height:1.3">${t.problem||'—'}</div>
        </div>
...
function openVerifySheet(tid) {
  ...
  document.getElementById('v-result-box').innerHTML = `
    ...
    ${t.parts ? `<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px 12px;margin-bottom:${hasAfter?'10px':'0'}">
      ...
      <div style="font-size:0.8rem;color:#334155;line-height:1.6">${t.parts}</div>
    </div>` : ''}
...
function openCloseConfirmSheet(tid) {
  ...
  document.getElementById('cc-info').innerHTML = `
    <div style="font-weight:700;font-size:0.95rem;color:var(--text)">${t.id} — ${t.problem}</div>
...
function openDetail(tid) {
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 16px 10px">
      <div style="display:flex;align-items:center;gap:8px">
        ...
        <div>
          ...
          <div style="font-size:0.78rem;font-weight:800;color:#0f172a">${t.problem}</div>
        </div>
      </div>
  ...
  <div style="padding:14px 0 0">
    ...
    <div style="border:1px solid #e5e7eb;border-left:3px solid #c8102e;border-radius:0 8px 8px 0;padding:10px 13px;margin-bottom:14px;background:white">
      ...
      <div style="color:#374151;font-size:0.82rem;line-height:1.65">${t.detail}</div>
    </div>`:''}
    ...
    <div style="border:1px solid #d1fae5;border-radius:10px;overflow:hidden;margin-bottom:14px;background:white">
      ...
      <div style="padding:10px 13px">
        ...
        ${t.parts?`<div style="font-size:0.75rem;color:#166534;background:#f0fdf4;border-radius:6px;padding:6px 10px;display:flex;gap:6px;align-items:flex-start"><span>🔩</span><span>อะไหล่: ${t.parts}</span></div>`:''}
  ...
  if(dayTickets.length) {
    html += dayTickets.map(t => `
      <div style="display:flex;align-items:center;gap:8px;background:white;border-radius:10px;padding:8px 10px;margin-bottom:6px;border-left:3px solid ${stColor[t.status]||'#ccc'};cursor:pointer" onclick="openTicketSheet('${t.id}')">
        <div style="flex:1;min-width:0">
          <div style="font-size:0.8rem;font-weight:700;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${t.problem}</div>
  ...
  el.innerHTML = msgs.map(m => {
    ...
    return `<div style="display:flex;gap:6px;align-items:flex-end;${isMe?'flex-direction:row-reverse':''}">
      ...
      <div style="max-width:72%">
        ${!isMe?`<div style="font-size:0.62rem;color:var(--muted);margin-bottom:2px;${isMe?'text-align:right':''}">${m.name}</div>`:''}
        <div style="background:${isMe?'var(--accent)':'white'};color:${isMe?'#fff':'var(--text)'};border-radius:${isMe?'14px 14px 4px 14px':'14px 14px 14px 4px'};padding:7px 11px;font-size:0.8rem;box-shadow:0 1px 4px rgba(0,0,0,0.08)">${m.text}</div>
...
function openCalEventSheet(dateStr, editId) {
  ...
  ms.innerHTML = '<option value="">— เลือกเครื่อง (ถ้ามี) —</option>' +
    macList.map(m=>`<option value="${m.id}"${ev?.machineId===m.id?' selected':''}>[${m.serial}] ${m.name}</option>`).join('');
...
function onCevDeptChange(dept) {
  ...
  ms.innerHTML = '<option value="">— เลือกเครื่อง (ถ้ามี) —</option>' +
    macList.map(m=>`<option value="${m.id}">[${m.serial}] ${m.name}</option>`).join('');
  ms.value = '';
